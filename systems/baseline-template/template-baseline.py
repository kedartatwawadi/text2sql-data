...
# IO
parser.add_argument('dataset', help='Data in json format', type=str)
...
args.data = os.path.join("../../data",args.dataset + ".json")
args.sql = os.path.join("../../data",args.dataset + "-sqlite.db")
print(args.sql)
conn = sqlite3.connect(args.sql)
dbengine = conn.cursor()
...
def build_vocab(sentences):
    ...
    tag_set = set()
    template_set = set()
    for tokens, tags, template,_ in train:
...
train,dev,test = create_dataset(args.data)
...
for tokens, tags, template, text_variables in test:
    _set = set(text_variables.keys())
    _set.add('O')
    template_to_tagset[template] = _set
...
def run_eval_old(data, builders, iteration, step):
    if len(data) == 0:
    ...
    templates_good = 0.0
    oracle = 0.0
    for tokens, tags, template,_ in data:
        word_ids = [vocab_words.w2i.get(word, UNK) for word in tokens]
        tag_ids = [0 for tag in tags]
        pred_tags, pred_template, _, _ = build_tagging_graph(word_ids, tag_ids, 0, builders, False)
        gold_tags = tags
        perfect = True
...
def run_eval(data, builders, iteration, step,k, template_to_tagset):
    print("Beam Size: ", k)
    if len(data) == 0:
        print("No data for eval")
        return -1
    ...
    for tokens, tags, template, text_variables in data:
        #if template_to_tagset[template] != set(tags):
            #print(tokens)
            #print(tags)
            #print(template)
            #print()
        ...
        for score, _tags, _template in final_topk:
            if template_to_tagset[_template] == set(_tags):
                ...
                try:
                    ...
                    dbengine.execute(sql_query)
                    out = dbengine.fetchall()
        ...
        if tags_correct and tag_set_correct and template_correct:
            complete_good += 1
...
# Final dev
if args.do_test_eval:
    run_eval(test, builders, "End", "test",1,template_to_tagset)
    run_eval(test, builders, "End", "test",5,template_to_tagset)
    run_eval(test, builders, "End", "test",10,template_to_tagset)
    run_eval(test, builders, "End", "test",20,template_to_tagset)
    run_eval_old(test, builders, "End", "test")

else:
    run_eval(test, builders, "End", "dev",1,template_to_tagset)
    run_eval(test, builders, "End", "dev",5,template_to_tagset)
    run_eval(test, builders, "End", "dev",10,template_to_tagset)
    run_eval(test, builders, "End", "dev",20,template_to_tagset)
    run_eval_old(test, builders, "End", "dev")
